...
## ADR-023
Date: 2026-07-05 · Decision: Certificate + verification design (T-014)
Context: Certificates are publicly verifiable; QR must resist forgery; institutions need machine access before T-017's full apimgmt.
Chosen: (1) `verification_no` is RANDOM (`EFP-` + 12 hex, ADR-008 non-enumerable); `certificate_no` stays sequential for humans. (2) QR payload = compact JSON {v,no,name,issued,sig}; sig = HMAC-SHA256(ABIS_QR_SECRET) over number|name|issued — the NAME is inside the signature (a tampered name must break verification; the first test round caught this). (3) Public verify returns 200 `{valid:false,status:"invalid"}` for unknown numbers (no 404 enumeration signal); masked name = first name + initials. (4) Minimal `apimgmt.ApiCredential` lands early (key = `<prefix>.<secret>`, sha256 hash stored, constant-time compare; key_hash in audit mask list); T-017 extends. (5) Decision endpoint requires in_review (supervisor/admin); explicit one-step advance endpoints (biometrics-captured, to-review) keep the ADR-021 machine intact rather than auto-jumping. (6) Every verification attempt logs a VerificationEvent incl. channel/ip/credential.
Future impact: T-017 builds credential CRUD on the same model; revocation endpoint + expiry sweep can come with T-020; frontend public verify page (T-018) consumes the masked shape.

## ADR-024
Date: 2026-07-05 · Decision: SMS notification pipeline (T-015)
Context: Applicants must hear about submitted/paid/ready without coupling registration to notifications; no SMS gateway locally.
Chosen: (1) `registration.signals.application_status_changed` emitted from services.transition after every successful move — notifications subscribes in ready() (same pattern as watchlist/matching). (2) `ClearanceApplication.contact_phone` added (applicant contact captured at intake; DATABASE_DESIGN updated) — triggers skip quietly when empty. (3) Templates ({placeholder} format_map w/ empty-string defaults) seeded by data migration; per-status map: submitted→application_submitted, paid→payment_received, certificate_issued→certificate_ready. (4) SmsProvider ABC selected via `ABIS_SMS_PROVIDER`; ConsoleSmsProvider logs in dev; dispatch via Celery (on_commit, eager-aware); failures keep the outbox row with status=failed+error for retry/reporting. (5) SmsMessage is NOT in the audit registry (it is itself an outbox log; auditing it would double-write); SmsTemplate is audited.
Future impact: real gateway = one provider class + env swap; T-016 reports can aggregate outbox stats; T-018 admin UI reads outbox.

## ADR-025
Date: 2026-07-05 · Decision: Reports registry + role-scoped KPIs (T-016)
Context: Reports must be safe (no user SQL), exportable in 3 formats, and dashboards differ per role.
Chosen: (1) ReportDefinition.query_key maps to a python builder in reports/builders.py returning ReportData(title/columns/rows/summary) — definitions are seeded data, custom SQL is out of scope by design. (2) Renderers: csv (utf-8-sig BOM for Excel), openpyxl xlsx (bold header), reportlab platypus pdf table. (3) Runs execute in Celery (queued→running→done|failed, file on media, error retained); downloads audited as EXPORT. (4) KPI endpoint returns role-scoped BLOCKS (ROLE_BLOCKS map in reports/services.py) — shape itself differs per role, tested per role. (5) `scheduled` cron field is storage-only until a beat integration lands. (6) spectacular note: identical choice sets (MatchJob.Status/ReportRun.Status) must share ONE override name or ENUM_NAME_OVERRIDES flags duplication.
Future impact: T-018 dashboard consumes the KPI blocks; T-019 seed feeds report data; beat scheduling is a small follow-up using `scheduled`.
